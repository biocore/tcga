...
def create_draft_tasks(api):
    ...
    for i in range(len(bam_inputs)):
        ...
        if gb > 400:
            ...
            total_files_tasked += len(gb_group_name)
            gb_group_name = []
            gb = []
            group = []
            bam_files_inputs = []
    ...
    print("total files tasked = %s" % str(total_files_tasked))
    print("total tasks created = %s" % str(task_counter))
