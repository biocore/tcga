...
def create_task_flagstat_cgc(all_files,
                             logger,
                             task_name,
                             config,
                             api):
    """Create CGC task for samtools reads count.
...
def create_tasks(api,
    ...
    """Create draft tasks for samtools-flagstat-worklfow workflow.
    ...
    for i, file in enumerate(sorted(bam_inputs)):
        ...
        if (total_size_gb + file_size_gb > upper_bound_group_size and
            ...
            task_name = "%s_%s_task_%s_files_%.2fGb" % (
                ...
                str(len(all_files)),
                total_size_gb)
            # Create draft tasks for samtools-flagstat-workflow workflow
            create_task_flagstat_cgc(all_files, logger, task_name, config,
                                      	   api)
            all_files = []
            total_size_gb = 0.0
